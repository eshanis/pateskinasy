...
STEP 1. The first command is to create the mysql network.
<br>
``docker network create mysql-network``

STEP 2. The next command is to run mysql in the docker container 
...
STEP 3. Then run PHPMyadmin on port 8080 using eshani-mysql as the host with the command 
<br>
...
STEP 4.  Jupyter is an option to interact with database in place of PHPMyadmin. The command to run Jupyter on mysql
